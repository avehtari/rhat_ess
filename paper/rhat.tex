...
iterations of warmup and 1000 iterations stored.
...
The efficiency of sampling is very low in the tails (which is
...
50 dimensional quantity $x$ is computed.  There are no warnings, and
...
\ref{fig:quantile-ess-alt1-1} show the good sampling efficiency for the
...
using the nominal parameterization.  We set independent half-Cauchy
distribution for 50 dimensional vector $x$ with positivity constraint
(\texttt{\textless{}lower=0\textgreater{}}).  Stan does then the
sampling automatically in the unconstrained \(\log(x)\) space, which
changes the geometry crucially.  There are no warnings, and all
\emph{split}-\(\widehat{R}<1.01\) and ESS\(>400\) indicate good
performance of the sampler.  More experiments for half-Cauchy
distribution can be found in the online appendix.
...
Online appendix contains more detailed analysis of different algorithm
...
model with an increased \texttt{adapt\_delta} value to reduce the
probability of getting divergent transitions. Despite an increased
\texttt{adapt\_delta}, we still observe a lot of divergent
transitions, which in itself is already sufficient indicator of
convergence problems. We can use \emph{split}-\(\widehat{R}\) and ESS
diagnostics to recognize problematic parts of the posterior, and they
can be used also in cases when other MCMC algorithms than HMC is used.
%
...
quantiles.
%
Red ticks, which show iterations with divergences, have
concentrated to small $\tau$ values, indicate also problems exploring
...
%
For reasons of comparability, we use the same \texttt{adapt\_delta} as
for the centered parameterization model. 
%
% \begin{verbatim}
% data {
%   int<lower=0> J;
...
% \end{verbatim}
%
There are zero divergences and no other warnings.
%
