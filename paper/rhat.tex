...
\maketitle
\begin{abstract}
  We extend $\widehat{R}$ diagnostic and effective sample size
  estimates, two tools we use to routinely monitor the convergence of
  iterative simulations.  As these are well defined only if the
  marginal posteriors have finite mean and variance, we propose to use
  rank normalized parameter values instead of the actual parameter
  values for the purpose of diagnosing convergence. We also introduce
