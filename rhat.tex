...
have finite variance and it is more stable more finite higher moments
exist. There is a problem that sometimes further derived quantities may have less existing finite moments. For example, if $\theta_1$ and $\theta_2$ are distributed as ${\mathrm N}(0,1^2)$ all higher moments exist and {\em split}-$\widehat{R}$ is stable, but ratio $\theta_1/\theta_2$ has Cauchy distribution which has undefined mean and infinite variance, and thus {\em split}-$\widehat{R}$ is not well defined. Given finite number of draws {\em split}-$\widehat{R}$ may seem to converge also in this case, but it's meaning is not clear.
% expectations of fractional moments of absolute values (or sqrt(1+x^2))?
