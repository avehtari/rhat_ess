...
The Split-$\widehat{R}$ statistic and the effective sample size
$S_{\rm eff}$ (ESS) (previously called $N_{\rm eff}$ or $n_{\rm eff}$)
...
## Efficiency estimate for the cumulative distribution function
...
## Efficiency estimate for quantiles {#quantile_S_eff}
...
## Efficiency estimate for median and MAD
...
- from the classic effective sample size estimate (Neff), which currently doesn't use
chain splitting, to rank-normalized-split-$S_{\rm eff}$ (bulk-ESS) and
...
- Rank plots
- Plots for efficiency estimates for quantiles
- Plots for efficiency estimated for small probability intervals
...
[Relative efficiency for quantiles](#quantile_S_eff)). Each interval has a
...
We examine the sampling efficiency in different parts of the
posterior by computing the relative efficiency for small interval
...
cat('lp__: Bulk-ESS = ', round(res['lp__', 'zsreff'], 2), '\n')
cat('lp__: Tail-ESS = ', round(res['lp__', 'zfsreff'], 2), '\n')
```

We can further analyze potential problems using local relative
efficiency and rank plots. We examine x[`r which_min_ess`], which has
the smallest bulk-ESS `r round(min(res$zsreff), 2)`.

We examine the sampling efficiency in different parts of the
posterior by computing the relative efficiency for small interval
probability estimates (see [Section Relative efficiency for small
...
[Section Relative efficiency for quantiles](#quantile_S_eff)). Each
...
```{r}
res <- monitor_extra(samp[, , 1:50])
which_min_ess <- which.min(res$zsreff)
```
...
variance. The smallest bulk-ESS is around $0.25$, which is not that
bad. The smallest median-ESS is larger than $0.5$, that is we are
able to estimate the median efficiently. However, many tail-ESS's are
...
similar problems in the plot of relative efficiency for quantiles.
...
which_min_ess <- which(res$zsreff == min(res$zsreff))
```
...
```{r}
cat('Mean Bulk-ESS for x =' , round(mean(res[, 'zsreff']), 2), '\n')
cat('Mean Tail-ESS for x =' , round(mean(res[, 'zfsreff']), 2), '\n')
cat('Mean Bulk-ESS for x_a =' , round(mean(res1[, 'zsreff']), 2), '\n')
cat('Mean Bulk-ESS for x_b =' , round(mean(res2[, 'zsreff']), 2), '\n')
```
...
ones. Some Bulk-ESS and median-ESS are larger than 1, 
...
We examine the sampling efficiency in different parts of the posterior by
computing the relative efficiency for small intervals for `tau`.
...
`r round(min(res[1:16, "zsreff"]), 2)`.  However tail-ESS for all
$x$ is less than `r round(max(res[1:16, "zfsreff"]), 2)`. Further, 
bulk-ESS for `lp__` is only `r round(res["lp__", "zsreff"], 2)`.  
...
posterior by computing the relative efficiency for small interval estimates 
for `x[1]`.
...
Let's check the relative efficiency for quantiles.
...
The mean of the bulk-ESS for $x_j^2$ is `r round(mean(res$zsreff),
...
computing the relative efficiency for small interval probability estimates for
`x[1]^2`.
...
part of the tail. Let's check the relative efficiency for quantiles.
