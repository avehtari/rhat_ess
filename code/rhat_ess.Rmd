...
use that to introduce efficiency diagnostics of quantiles and
...
[Relative efficiency of quantiles](#quantile_S_eff)). Each interval has a
...
We may also investigate how the estimated effective sample size changes when we
...
In the plot below, we see how unstable both bulk and tail efficiencies are for
this example.

```{r, cache=FALSE}
plot_change_reff(fit = fit_nom, par = which_min_eff)
```
...
```{r, cache=FALSE}
plot_change_reff(fit = fit_cp, par = "tau")
```
...
[Section Relative efficiency of quantiles](#quantile_S_eff)). Each
...
```{r, cache=FALSE}
plot_change_reff(fit = fit_cp2, par = "tau")
```
