...
In order to estimate the efficiency in the distributions' tails, we propose to
compute the minimum of the effectve sample sizes of the 5% and 95% quantiles. As
a shorthand, we use the term *tail effective sample size* (*tail-ESS*). Tail-ESS
is also useful for diagnosing problems due to different scales of the chains
(see [Appendix D](#AppendixD)).
...
TODO: Add more differentiated recommendations about bulk and tail ESS?
Or just recommend both being above 400?
...
smallest bulk-ESS of `r round(min(mon[,'Bulk_ESS']), 2)`. 
...
autocorrelation. We plot relative efficiency $R_{\rm eff}=S_{\rm
eff}/S$ for easier comparison between different values of $S$.
...
ggplot(data = res, aes(y = zsreff, x = trend)) + 
  ...
  geom_hline(aes(yintercept = 400 / iters), linetype = 'dashed') + 
...
```{r}
ggplot(data = res, aes(y = tailseff, x = scale)) + 
  geom_point() + 
  geom_jitter() + 
  facet_grid(. ~ iters) + 
...
**Result:** The tail effective sample size detects the difference in
scales as it focuses on the scale of the chains.
...
the smallest bulk-ESS of `r round(min(res$zsseff), 2)`.
...
relative Bulk-ESS for `tau` is very small 0.5%.
...
Let's check what happens if we run even longer chains and then thin the draws to
reduce autocorrelation. With 400000 iterations per chain, thinning of 200 and 4
chains, we again end up with 4000 iterations as with the default settings.
...
We still observe some divergent transitions which potentially bias the results.
Apart from that, the convergence diagnostics look ok. 
...
Various diagnostic plots of $\tau$ look reasonable as well.
```{r}
...
Apparently, after substantial efforts, we got the centered parameterization to
yield better results. However, the estimate of $\tau$ is still around 3 sigmas
away from the corresponding estimate of the non-centered parameterization.
