...
Split-$\widehat{R}$ and effective sample size ($N_{\rm eff}$), are routinely used
...
effective sample size estimates as implemented in Stan 2.18 [@Stan:JSS:2017; @Stan.2.18].

## *Split*-$\widehat{R}$

Below, we present the computation of *Split*-$\widehat{R}$ following
the notation of @BDA3.  For each scalar summary of interest $\psi$, we
compute $B$ and $W$, the between- and within-sequence variances:
$$
...
multiple chains $j \in (1,\ldots,m)$ are combined with the within-chain
...
$$
R_{\rm eff}=\textit{rank-normalized-split-}N_{\rm eff} / N,
$$
...
We intentionally separate MCMC diagnostics based on generic and stable
rank-normalized relative efficiency $\hat{N}_{\rm eff}/N$ and Monte
Carlo error estimates for the desired quantities, as the latter is
only applicable when mean and variance of this quantity exist. We
could use Pareto-$k$-diagnostics to estimate the existance of mean and
variance [@Vehtari+etal:PSIS:2017], but this is out of the scope of
...
- Rank normalization makes zsRhat and zsreff well defined for all
  distributions, under bijective transformations invariant and more stable
...
The code for alternative new *split*-$\widehat{R}$ and $N_{\rm eff}$ versions are in [monitornew.R](monitornew.R).
