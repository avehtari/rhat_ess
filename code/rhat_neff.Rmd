...
author: "Aki Vehtari, Andrew Gelman, Daniel Simpson, Paul Bürkner"
date: "August 2018"
encoding: "UTF-8"
output:
  html_document:
...
simulations. The effective sample size estimate ($N_{\rm eff}$) by 
Gelman et al. (2013) is based on marginal variance and autocorrelation 
estimates from split chains. 

<!--
I would suggest formally defining the introduced statistics.
Otherwise it is hard to understand what follows for readers
not familiar with Rhat and Neff.
-->

## *Split*-$\widehat{R}$

Here are the formulas, with the presentation taken from Gelman et al.\
(2013).  For each scalar summary of interest $\psi$, we compute $B$
...
Effective sample size $N_{\rm eff}$ can be larger than
...
produce $N_{\rm eff}>N$ for parameters which have close
...
If a marginal distribution of some parameter does not have finite 
mean and variance, $\widehat{R}$, split-$\widehat{R}$ and 
$N_{\rm eff}$ for that parameter are not well defined.
To have a well defined MCMC convergence diagnostic and generic
relative MCMC efficiency estimate, we propose to compute
split-$\widehat{R}$ and $R_{\rm eff}=N_{\rm eff} / N$ 
(where $N$ is the total number of draws) for rank normalized
...
applies z-transformation to those ranks, and reconstruct the chains. 
Rank normalization makes the subsequent computations well defined and
bijectively invariant. This means that we get same results, for
example, if we use unconstrained or constrained parameters. Furthermore,
...
<!--
Invariant under what kind of transformations? please clarify

Clarify what "reconstructs the chains" means.
-->

## Relative efficiency of median and other quantile estimates

Classic $N_{\rm eff}$ focuses on posterior mean, but as the mean 
can be non-finite, we may prefer to use the median and other
quantiles by default. If the median of $x$ is $\tilde{x}$, 
then by definition $p(x < \tilde{x}) = 0.5$. 
...
absolute deviation based scale estimate (MAD_SD), by computing the relative
efficiency for median of absolute deviations from the median of all draws.

<!--
The above sentence is hard to understand. 
Maybe also explain with some equations?
-->
...
We illustrate that rank plots can be used as a more compact alternative
to trace plots.
...
<!--
Is this max_treedepth thing really necessary for this case study?
It slows down sampling so drastically that noone ever wants
to run this code...
-->
...
```{r}
mcmc_hist_r_scale(samp[, , which_min_eff])
```
