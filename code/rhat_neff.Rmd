...
The follwing function computes MCSE, interval and S_eff for desired quantile of a quantity. 
```{r}
...
plot_local_reff(samp = samp_cp, par = "tau", nalpha = 20)
```
...
```{r}
plot_quantile_reff(samp = samp_cp, par = 2, nalpha = 40)
```
...
plot_local_reff(samp = samp_cp2, par = 2, nalpha = 50)
```
...
We examine also the relative efficiency of different quantile estimates.
```{r}
plot_quantile_reff(samp = samp_cp2, par = 2, nalpha = 100)
```
...
```{r}
plot_local_reff(samp = samp_cp2, par = 1, nalpha = 50)
```
...
```{r}
plot_quantile_reff(samp = samp_cp2, par = 1, nalpha = 100)
```
...
plot_local_reff(samp = samp_cp3, par = 2, nalpha = 100)
```
...
We examine also the relative efficiency of different quantile estimates.
```{r}
plot_quantile_reff(samp = samp_cp3, par = 2, nalpha = 100)
```
...
plot_local_reff(samp = samp_ncp, par = 2, nalpha = 20)
```
...
We examine also the relative efficiency of different quantile estimates.
```{r}
plot_quantile_reff(samp = samp_ncp, par = 2, nalpha = 40)
```
...
plot_local_reff(samp = samp_ncp2, par = 2, nalpha = 20)
```
...
```{r}
plot_local_reff(samp = samp_ncp2, par = 2, nalpha = 50)
```
...
We examine also the relative efficiency of different quantile estimates.
```{r}
plot_quantile_reff(samp = samp_ncp2, par = 2, nalpha = 40)
```
...
plot_local_reff(samp = samp_ncp3, par = "tau", nalpha = 100)
```
...
We examine also the relative efficiency of different quantile estimates.
```{r, cache=FALSE}
plot_quantile_reff(samp = samp_ncp3, par = "tau", nalpha = 100)
```

Rank plot visualisation of `tau`, which has the smallest relative
efficiency of `r round(monitor_simple(samp_ncp3[, , "tau"])$zsreff, 2)` among parameters.
```{r}
