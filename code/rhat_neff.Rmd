...
## Effective sample size $\widehat{S}_{\rm eff}$ {#Seff}
...
$$
\widehat{S}_{\rm eff} = \frac{N \, M}{\hat{\tau}},
$$
where
$$
...
$\widehat{S}_{\rm eff}$ described here is different from similar formulas in
...
stuck in different modes), then our $\widehat{S}_{\rm eff}$ is close to
the number of chains.

In Stan pre 2.18, a slightly incorrect initial sequence was used, which 
implied that $\widehat{S}_{\rm eff}$ was capped at $S$. As
antithetic behavior $\widehat{S}_{\rm eff} > S$ is not that common or the
...
1. Ranking: Replace each value $\theta^{(nm)}$ by its rank $r^{(nm)}$. Average
...
2. Normalization: Normalize ranks by inverse normal transformation $z^{(nm)} =
\phi^{-1}((r^{(nm)}-1/2)/S)$. 

For continuous variables and $S \rightarrow \infty$, the rank normalized values are normally distributed and rank normalization performs non-parametric transformation to normal distribution. Using normalized ranks instead of ranks directly, has the benefit that the behavior of $\widehat{R}$ and $\widehat{S}_{\rm eff}$ do not change for normally distributed $\theta$.
...
invariant estimate of the mizing efficiency of chains. For simplicity
...
form of expectation, and thus $\widehat{R}$ and $\widehat{S}_{\rm
...
*rank-normalized-folded-split*-$\widehat{S}_{\rm eff}$.
...
$aS$ and $bS$.
3. Use the corresponding $\theta^{(s)}$ as draws from Monte Carlo error
...
(which is caused by slow mixing in long tails of Cauchy)..
...
```{r plot_quantile_reff, cahce=FALSE}
...
Rank plot visualisation of x[1], which has bulk relative efficiency 
...
bulk relative efficiency of `r round(min(res$zsreff), 2)`, 
