...
@BDA3, but using the notation style of .  For each scalar summary of interest $\theta$, we
compute $B$ and $W$, the between- and within-chain variances:
$$
B = \frac{n}{m-1}\sum_{j=1}^{m}(\overline{\theta}_{.j}-\overline{\theta}_{..})^2,
\;\mbox{ where }\;\;\overline{\theta}_{.j}=\frac{1}{n}\sum_{i=1}^n \theta_{ij},\;\;
\;\;\overline{\theta}_{..} = \frac{1}{m}\sum_{j=1}^m\overline{\theta}_{.j}\\
W =
 \frac{1}{m}\sum_{j=1}^{m}s_j^2, \;\mbox{ where }\;\; s_j^2=\frac{1}{n-1}
\sum_{i=1}^n (\theta_{ij}-\overline{\theta}_{.j})^2.
$$
...
the within-chain means, $\overline{\theta}_{.j}$, each of which 
is an average of $n$ values $\theta_{ij}$.
...
## Effective sample size $\widehat{N}_{\rm eff}$ {#Neff}
...
simulations $\bar{\theta}_{..}$ as an estimate of the posterior mean
...
$$
\widehat{N}_{\rm eff} = \frac{n \, m}{\hat{\tau}},
$$
where
$$
...
$\widehat{N}_{\rm eff}$ described here is different from similar formulas in
...
stuck in different modes), then our $\widehat{N}_{\rm eff}$ is close to
...
implied that $\widehat{N}_{\rm eff}$ was capped at $N$. As
antithetic behavior $\widehat{N}_{\rm eff} > N$ is not that common or the
...
1. Ranking: Replace each value $\theta_{ij}$ by its rank $r_{ij}$. Average
...
2. Normalization: Normalize ranks by inverse normal transformation $z_{ij} =
\phi^{-1}((r_{ij}-1/2)/N)$. 

For continuous variables and $N \rightarrow \infty$, the rank normalized values are normally distributed and rank normalization performs non-parametric transformation to normal distribution. Using normalized ranks instead of ranks directly, has the benefit that the behavior of $\widehat{R}$ and $\widehat{N}_{\rm eff}$ do not change for normally distributed $\theta$.
...
Rank normalized *Split*-$\widehat{R}$ is computed using the equations in [Section *Split*-$\widehat{R}$](#SplitRhat) by replacing parameter values $\theta_{ij}$ with rank normalized values $z_{ij}$.
...
median
$$
  {\rm abs}(\theta_{ij}-{\rm median}(\theta)).
$$
...
invariant estimate the efficiency of mixing of chains. For simplicity
we propose to report relative efficiency values
$$
R_{\rm eff}=\textit{rank-normalized-split-}N_{\rm eff} / N,
$$
where $\textit{split-}N_{\rm eff}$ is computed using equations in
[Section Effective sample size](#Neff) by replacing parameter values
$\theta_{ij}$ with rank normalized values $z_{ij}$. For $\theta$ which
...
introduce relative efficiency of quantiles and even more local
...
form of expectation, and thus $\widehat{R}$ and $\widehat{N}_{\rm
eff}$ equations are not directly applicable. Thus we first focus on
...
$$
p(\theta<\theta^*) \approx \bar{I} = \frac{1}{N}\sum_{i=1}^N
I(\theta<\theta^*),
$$
where $I(\theta<\theta^*)$ is the indicator function. The indicator
...
draws and $Y=\sum_{i=1}^N I(\theta<\theta^*)$, this estimate has ${\rm
Beta}(Y+1, N - Y + 1)$ distribution. Thus we can easily examine
variation of ECDF for any $\theta^*$ value from single chain. If $Y$
is not very close to $1$ or $N$ and $N$ is large, we can use the
variance of Beta distribution
$$
{\rm Var}[p(\theta<\theta^*)] = \frac{(Y+1)*(N-Y+1)}{(N+2)^2(N+3)}.
$$
...
for practical purposes in MCMC diagnostics for most purposes. By using
the relative efficiency equations for several split chains and the
indicator function $I(\theta<\theta^*)$, we can compute the relative
...
Assuming the cumulative distribution function $F$ is continuous near
an $\alpha$-quantile and we know $F$, we could use delta method to
compute a variance estimate for $F_\theta^{-1}(\bar{I})$. Although we
don't usually know $F$, delta method approach reveals that the the
...
of ECDF (via the indicator function $I(\theta<\theta^*)$) also for the
...
*rank-normalized-folded-split*-$\widehat{N}_{\rm eff}$.
...
efficiency also for an estimation of the scale.
...
1. Sample draws from the above mentioned Beta distribution or compute
two sigma points ($a=\bar{I}-{\rm sd}[\bar{I}]$, $b=\bar{I}+{\rm
sd}[\bar{I}]$) based on the normal approximation.
2. Find posterior draws with closest corresponding ranks. For example,
in case of sigma points, find $r_a$ and $r_b$ which are closest to
$aN$ and $bN$.
3. Use the corresponding $\theta_i$ as draws from Monte Carlo error
distribution or compute approximate standard error as ${\rm
abs}(\theta_a-\theta_b)/2$.
...
small intervals. We propose to compute the relative efficiency for
$$
...
The proposal is to add to bayesplot following
...
plot_reff <- function(res) {
  ...
  p3 <- ggplot(res, aes(x=par, y=zfsreff)) + 
    geom_point() + 
    ggtitle('Folded rel. efficiency (zfsreff)') + 
    geom_hline(yintercept = c(0,1)) + 
    geom_hline(yintercept = 0.1, linetype = 'dashed') + 
    scale_y_continuous(breaks = ybreaks, limits = ylimits)
...
cat('Mean fr_eff for xs = ' , round(mean(res[,'zfsreff']), 2), '\n')
