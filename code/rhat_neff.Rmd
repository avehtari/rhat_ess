...
We propose to report the maximum of
*rank-normalized-split*-$\widehat{R}$
and *folded-rank-normalize-split*-$\widehat{R}$.

## Relative efficiency of mean rank

As a generic relative MCMC efficiency estimate and to make more clear
the difference to the previously reported effective sample size, we
propose to report $R_{\rm eff}=\textit{split-rank-normalized-}N_{\rm eff}
/ N$ (where $N$ is the total number of draws).
...
*split*-$\widehat{R}$, we propose to plot rank plots for each chain
...
We intentionally separate MCMC diagnostic based on generic and
