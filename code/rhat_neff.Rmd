...
date: "September 2018 (first version May 2018)"
encoding: "UTF-8"
output:
  html_document:
    fig_caption: yes
...
to compute *split*-$\widehat{R}$ and $N_{\rm eff}$ using rank
normalized values, empirical cumulative density functions for ranks (RECDF), and small
posterior intervals.
...
estimates
 - to use relative efficiency of several rank ECDFs or small interval probability estimates as local relative efficiency measures to reveal local sampling problems
 - to use local efficiency and rank plots as a more compact alternative to trace plots.
...
1. Ranking: Replace each value $\theta_i$ by its rank $r_i$. Average
rank for ties are used to conserve the number of values in case of discrete
quantities. Ranks are computed jointly for all draws from all chains.
2. Normalization: Normalize ranks by inverse normal transformation $z_i =
\phi^{-1}((r_i-1/2)/N)$.

<!--
What's the particular normalization procedure being applied?
-->
...
## Relative efficiency of empirical CDF
...
could use Pareto-$k$-diagnostics to estimate the existence of mean and
...
efficiency measure which is independent on the shape of the
distribution.
...
dynamic HMC specific diagnostics [@betancourt2017conceptual]. Let's
see what generic Split-Rhat and relative efficiency can detect.
...
Treedepth exceedence and Bayesian Fraction of Missing Information are
dynamic HMC specific diagnostics [@betancourt2017conceptual]. Let's
see what generic Split-Rhat and relative efficiency can detect. Check
the diagnostics for all x's.
