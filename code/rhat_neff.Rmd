...
Effective sample size $S_{\rm eff}$ can be larger than $N$ in case of
...
can produce $S_{\rm eff}>N$ for parameters which have close to Gaussian 
posterior with low dependency on other parameters (before version 2.18, 
Stan computed $S_{\rm eff}$ so that the maximum reported value was $N$).
...
values of $S_{\rm eff}$) compared to purely between-chain
...
$$
R_{\rm eff}=\textit{rank-normalized-split-}S_{\rm eff} / N,
$$
