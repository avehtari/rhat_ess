...
Split-$\widehat{R}$ and effective sample size ($N_{\rm eff}$) are routinely 
used to monitor the convergence of iterative simulations. $\widehat{R}$
(Gelman and Rubin, 1992; Brooks and Gelman, 1998) and
...
effective sample size estimate by Gelman et al. (2013)
is based on marginal variance and autocorrelation estimates from split
chains.

$\widehat{R}$, *split*-$\widehat{R}$, and $N_{\rm eff}$ are well
defined only if the marginal distributions have finite mean and variance.
and even in that case, they are less stable for distributions with long
tails. To alleviate these problems, we propose to compute *split*-$\widehat{R}$ 
and $N_{\rm eff}$ using rank normalized values, quantiles, and 
posterior intervals.

We additionally propose

 - to diagnose convergence and relative efficiency of the marginal
distribution's scale in addition to the usual location estimate 
(mean or median), which will improve reliability of Monte Carlo error
estimates
 - to use relative efficiency of several quantile or small interval probability estimates as local relative efficiency measures to reveal local sampling problems
...
effective sample size estimates as implemented in Stan 2.18 (ref).

## *Split*-$\widehat{R}$

Below, we present the computation of *Split*-$\widehat{R}$ following 
the notation of Gelman et al. (2013). For each scalar summary of 
interest $\psi$, we compute $B$ and $W$, the between- and within-chain
variances:
$$
...
In the above equations, $n$ is the number of simulations per chain and 
$m$ is the number of chains. The between-chain variance, $B$, 
also contains the factor $n$ because it is based on the variance of 
the within-chain means, $\overline{\psi}_{.j}$, each of which 
is an average of $n$ values $\psi_{ij}$.

<!--
I replaced "sequence" with "chain" as I believe this is easier
to understand for most readers. If you don't agree, I am happy 
to revise it to "sequence".
-->
...
$W$ should *underestimate* $\mbox{var}(\psi|y)$ because the 
...
current distribution for $\psi$ might be reduced if the simulations 
...
and effective sample size in particular is provided in Geyer (2011).
The particular calculations used by Stan follow those for split-$\hat{R}$,
which involve both between-chain (mean) and within-chain calculations
(autocorrelation). They were introduced in the Stan manual and explained
...
$\mbox{E}(\psi|y)$. This can be a reasonable baseline even though it is
...
lags. Dynamic Hamiltonian algorithm used in Stan can produce $N_{\rm
eff}>N$ for parameters which have close to Gaussian posterior with low
dependency on other parameters (before version 2.18, Stan computed 
$N_{\rm eff}$ so that the maximum reported value was $N$).
...
from the samples themselves.  The rest of this section describes am
...
chain $j\in (1,\ldots,m)$ are combined with the within-chain
...
$$
\hat{\rho}_t
= 1 - \frac{\displaystyle W - \textstyle \frac{1}{m}\sum_{j=1}^m \hat{\rho}_{t,j}}{\widehat{\mbox{var}}^{+}}. \label{rhohat}
$$
...
$$
\hat{N}_{\rm eff} = \frac{N \, m}{\hat{\tau}},
$$
where
$$
...
the literature in that we use also between-chain variance in the
computation which typically gives us more conservative claims (lower
values of $N_{\rm eff}$) compared to purely between-chain
...
<!--
In the above paragraph using between-chain variance twice
doesn't make sense. Is one of those a typo?
-->

In Stan pre 2.18, slightly incorrect initial sequence was used, which had
the effect that the $\hat{N}_{\rm eff}$ estimate was capped at $N$. As
...
<!--
What's the particular normailization procedure being applied?
-->
...
Furthermore, to improve sensitivity to scale differences
...
the same location. To alleviate this, we propose to compute
*Split*-$\widehat{R}$ also for folded rank normalized values.
Folded values are obtained by applying the absolute deviation transformation
\begin{equation}
  {\rm abs}(x-{\rm median}(x)),
\end{equation}
where the median is computed from the draws from all chains. Folded rank
...
*folded-rank-normalized-split*-$\widehat{R}$ for each parameter.

## Relative efficiency of mean rank

In addition of using rank-normalized values for convergence
diagnostics, we can also compute the corresponding effective sample.
This estimate will be well defined even if the original
distribution does not have finite mean and variance, but it is not
directly applicable to estimate the Monte Carlo error of the
mean of the original values. We could transform quantiles the
...
<!--
I don't fully understand the sentence starting with "we could transform"
-->
...
absolute deviation (MAD) instead of mean and standard error
(SE). Median and MAD are well defined even when the marginal
distribution does not have finite mean and variance.
...
$$
\frac{1}{S}\sum_{s=1}^S I_{x_s<{\rm median}(x)}(x_s),
$$ 
where $S$ is the number of draws.
...
$p(x < \hat{Q}_{0.5})$, where $\hat{Q}_{0.5}$ is the empirical median from the combined draws. This should be good approximation, if the efficiency for 
...
measue as it only assesses how the simulation jumps around the median.
...
approach as for median but with the folded values also used in
$\textit{split-rank-normalized-}\widehat{R}$.
...
measures as demonstrated in the experiments.

## Relative efficiency of small interval probability estimates

We can get even more local relative efficiency estimate by considering small intervals. We propose to compute the relative efficiency for 
$p(\hat{Q}_\alpha \le x < \hat{Q}_{\alpha+\delta})$, where 
$\hat{Q}_\alpha$ is the empirical $\alpha$-quantile, $\delta=1/k$ is the 
length of the interval with some positive integer $k$ chosen by the researcher, and $\alpha \in (0,\delta,\ldots,1-\delta)$ changes in steps of $\delta$. 
Each interval has $N/k$ draws, and the efficiency measures 
...
<!--
Should we add more explanation of what a "rank plot" is?
-->

## Finite mean and variance of a marginal distribution

We intentionally separate MCMC diagnostics based on generic and
stable rank-normalized relative efficiency $\hat{N}_{\rm eff}/N$ and
Monte Carlo error estimates for desired quantities, as the latter is
only applicable when mean and variance of this quantity exist. We could use 
Pareto-$k$-diagnostics to estimate the existance of mean and variance 
(Vehtari, Gabry, & Gelman, 2017), but this is out of the scope of
the present case study. In the future, there will be a separate case study 
for diagnostics to check whether a certain marginal distribution has finite
mean and variance required to obtain reasonable estimates of 
posterior mean and Monte-Carlo standard error.
...
The proposal is to switch in Stan
 ...
 folded-rank-normalized-split-$\widehat{R}$: max(zsRhat, fzsRhat)
...
- Rank normalization makes zsRhat and zsreff well defined for all
  distributions, invariant under bijective transformations and more stable
  than sRhat and neff. Adding a folded version fzsRhat helps to detect
...
- Relative efficiencies of the median and MAD (medsreff and masreff) are
  ...
  more stable than neff, and focuse on the quantities we are acutally 
  reporting.
