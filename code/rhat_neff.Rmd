---
title: "Rank-normalized split-Rhat and relative efficiency"
author: "Aki Vehtari"
date: "May 2018"
output:
  html_document
...
In the code and figures I use abbreviations
 ...
 - neff = no-split effective sample size using Geyer's monotonic truncation rule
 - reff = neff / N
 - zsneff = rank-normalized split effective sample size using Geyer's monotonic truncation rule
 - zsreff = zsneff / N
 
The proposal is to switch 

 - from sRhat to max(zsRhat, fzsRhat)
 - from neff to zsreff
...
I also propose following warning thresholds
 ...
 - 1.01 for new Rhat max(zsRhat, fzsRhat)
 - 0.1 for new relative efficiency (zsreff)
...
## Trend: All chains from the same $N(0,1)$ distribution plus linear trend added to all
```{r}
...
*Result:* If we use threshold 1.01, we can detect shift with magnitude one third of marginal standard deviation. If we use threshold 1.1, we can detect shift with the magnitude equal to marginal standard deviation.
...
The following Cauchy models are originally from Michael Betancourt's case study [Fitting The Cauchy Distribution](https://betanalpha.github.io/assets/case_studies/fitting_the_cauchy.html)
...
zsreffs <- double()
parami <- 1
for (alpha in seq(0, .95, by=0.05)) {
  rs <- monitornew(samp[,,parami]>=quantile(samp[,,parami],alpha)&samp[,,parami]<quantile(samp[,,parami],alpha+0.05),warmup = 0, probs = 0.5, digits_summary = 2, print = FALSE)
  zsreffs <- c(zsreffs, rs[1,'zsreff'])
}
ggplot(data=data.frame(quantile=seq(0, 1, by=0.05), zsreff=c(zsreffs,zsreffs[20])), aes(x=quantile, y=zsreff))+ geom_step() + ylim(c(0,1)) + geom_hline(yintercept = c(0,1)) + geom_hline(yintercept = 0.1, linetype = 'dashed') + scale_x_continuous(breaks=seq(0,1, by=0.1))
```
...
Increasing chain length did not seem to change the relative efficiency.
With more draws from the longer chains we can use also finer resolution for the local efficiency estimates.
```{r}
zsreffs <- double()
parami <- 1
for (alpha in seq(0, .99, by=0.01)) {
  rs <- monitornew(samp[,,parami]>=quantile(samp[,,parami],alpha)&samp[,,parami]<quantile(samp[,,parami],alpha+0.01),warmup = 0, probs = 0.5, digits_summary = 2, print = FALSE)
  zsreffs <- c(zsreffs, rs[1,'zsreff'])
}
ggplot(data=data.frame(quantile=seq(0, 1, by=0.01), zsreff=c(zsreffs,zsreffs[100])), aes(x=quantile, y=zsreff))+ geom_step() + ylim(c(0,1)) + geom_hline(yintercept = c(0,1)) + geom_hline(yintercept = 0.1, linetype = 'dashed') + scale_x_continuous(breaks=seq(0,1, by=0.1))
```

The efficiency far in tails is worryingly low.
...
ggplot(data=data.frame(quantile=seq(0, 1, by=0.05), zsreff=c(zsreffs,zsreffs[20])), aes(x=quantile, y=zsreff))+ geom_step() + geom_hline(yintercept = c(0,1)) + geom_hline(yintercept = 0.1, linetype = 'dashed') + scale_x_continuous(breaks=seq(0,1, by=0.1)) + scale_y_continuous(breaks = seq(0,1,by=0.25), limits = c(0,1.1)) 
```
...
The models are from Michael Betancourt's case study on [Diagnosing Biased Inference with Divergences](http://mc-stan.org/users/documentation/case-studies/divergences_and_bias.html).
...
### Centered parameterization longer chains
...
zsreffs <- double()
parami <- 2
for (alpha in seq(0, .975, by=0.025)) {
  rs <- monitornew(samp[,,parami]>=quantile(samp[,,parami],alpha)&samp[,,parami]<quantile(samp[,,parami],alpha+0.025),warmup = 0, probs = 0.5, digits_summary = 2, print = FALSE)
  zsreffs <- c(zsreffs, rs[1,'zsreff'])
}
ggplot(data=data.frame(quantile=seq(0, 1, by=0.025), zsreff=c(zsreffs,zsreffs[40])), aes(x=quantile, y=zsreff))+ geom_step() + geom_hline(yintercept = c(0,1)) + geom_hline(yintercept = 0.1, linetype = 'dashed') + scale_x_continuous(breaks=seq(0,1, by=0.1)) + scale_y_continuous(breaks = seq(0,1,by=0.25), limits = c(0,1.1)) 
```
...
ggplot(data=data.frame(quantile=seq(0, 1, by=delta), zsreff=c(zsreffs,zsreffs[1/delta])), aes(x=quantile, y=zsreff))+ geom_step() + ylim(c(0,1)) + geom_hline(yintercept = c(0,1)) + geom_hline(yintercept = 0.1, linetype = 'dashed') + scale_x_continuous(breaks=seq(0,1, by=0.1))
```

There are gaps of poor efficiency which indicates sticking of MCMC, but the sticking doesn't happen for any specific values of mu.
...
ggplot(data=data.frame(quantile=seq(0, 1, by=0.01), zsreff=c(zsreffs,zsreffs[100])), aes(x=quantile, y=zsreff))+ geom_step() + geom_hline(yintercept = c(0,1)) + geom_hline(yintercept = 0.1, linetype = 'dashed') + scale_x_continuous(breaks=seq(0,1, by=0.1)) + scale_y_continuous(breaks = seq(0,1,by=0.25), limits = c(0,1.1)) 
```
