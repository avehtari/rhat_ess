...
ggplot(data=data.frame(quantile=seq(0, 1, by=delta), zsreff=c(zsreffs,zsreffs[nalpha])), aes(x=quantile, y=zsreff))+ geom_step() + ylim(c(0,1)) + geom_hline(yintercept = c(0,1)) + geom_hline(yintercept = 0.1, linetype = 'dashed') + scale_x_continuous(breaks=seq(0,1, by=0.1))
```
...
ggplot(data=data.frame(quantile=seq(0, 1, by=delta), zsreff=c(zsreffs,zsreffs[nalpha])), aes(x=quantile, y=zsreff))+ geom_step() + geom_hline(yintercept = c(0,1)) + geom_hline(yintercept = 0.1, linetype = 'dashed') + scale_x_continuous(breaks=seq(0,1, by=0.1)) + scale_y_continuous(breaks = seq(0,1,by=0.25), limits = c(0,1.1)) 
```
...
Mean of the relative efficiencies for $x[j]^2$ is 0.4, which is quite close 
to the relative efficiency for `lp__`. This is not that surprising as the potential energy in normal model is relative to $\sum_{j=1}^J x_j^2$.
