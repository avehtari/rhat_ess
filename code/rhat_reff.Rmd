...
- if computing median and MAD_SD, report the corresponding $R_{\rm eff}$'s
...
- Relative efficiencies of the median and MAD are well defined for all
distributions, invariant under bijective transformations, and more stable than
classical measures of effective sample size. Also they focus on the quantities 
we are actually reporting.

In summary outputs, we propose to eventually also use `Rhat` to denote the new
version. However, to make it more explicit that the rank-normalized efficiency
is different than standard efficiency, we could use the term `Reff` to denote
relative efficiency instead of using `Neff` for the effective sample size (i.e.
absolute efficiency). The relative efficiency is also easier to check for low
values as we don't need to compare it to the total number of draws.
...
Based on the experiments presented in the appendices, more strict convergence
...
following warning thresholds although additional experiments would be useful:
 ...
 - 1.01 or 1.02 for the new Rhat
 - 0.1 for the new relative efficiency
...
We already looked at the nominal Cauchy model with direct parameterization 
in the main text, but for completeness, we take a closer look here again.
...
data, which we started in [Section Eight Schools](#eightschools).
...
As we observed clear problems, we shouldn't trust any Monte Carlo standard 
error (MCSE) estimates. However, for illustrationary purposes, we still 
compute the MCSE, tail quantiles and corresponding Seff for the median of 
`mu` and `tau`. Comparing to the shorter MCMC run, 10 times more draws has not 
reduced the MCSE to one third as would be expected without problems in the
...
is not something we would recommend doing in practice, as it is very unlikely to
solve any problems as illustrated below.
