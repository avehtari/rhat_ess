...
```{r}
mon <- monitorn(fit_nom)
which_min_eff <- which(mon[,'Bulk-Reff'] == min(mon[,'Bulk-Reff']))
```
...
the smallest bulk relative efficiency `r round(min(mon[,'Bulk-Reff']), 2)`.
...
plot_local_reff(samp = samp, par = which_min_eff, nalpha = 20)
```
...
```{r, cahce=FALSE}
plot_quantile_reff(samp = samp, par = which_min_eff, nalpha = 40)
```
We see that the efficiency of MCMC is worryingly low in the tails
(which is caused by slow mixing in long tails of Cauchy).
...
```{r}
mon <- monitorn(fit_alt1)
which_min_eff <- which(mon[101:150,'Bulk-Reff'] == min(mon[101:150,'Bulk-Reff']))
```
...
plot_local_reff(samp = samp, par = 100+which_min_eff, nalpha = 20)
```
...
```{r}
plot_quantile_reff(samp = samp, par = 100+which_min_eff, nalpha = 40)
```
